...
function highlightTableCells(tableId, hoverClass, clickClass, multiple) {
    ...
    if (hoverClass) {
        ...
        table.onmouseover = table.onmouseout = function (e) {
            ...
            if (elem.parentNode.tagName == 'TR' && elem.parentNode.parentNode.tagName == 'TBODY') {
                var row = elem.parentNode;//ряд содержащий ячейку таблицы в которой произошло событие
                //Если текущий ряд не "кликнутый" ряд, то в разисимости от события либо применяем стиль, назначая класс, либо убираем.
                if (!elem.getAttribute('clickedRow')) elem.className = e.type == "mouseover" ? elem.className + " " + hoverClass : elem.className.replace(hoverClassReg, " ");
            }
        };
    }
    ...
    if (clickClass) table.onclick = function (e) {
        ...
        if (elem.parentNode.tagName == 'TR' && elem.parentNode.parentNode.tagName == 'TBODY') {
            ...
            //Если текущий ряд уже помечен стилем как "кликнутый"
            if (elem.getAttribute('clickedRow')) {
                elem.removeAttribute('clickedRow');//убираем флаг того что ряд "кликнут"
                elem.className = row.className.replace(clickClassReg, "");//убираем стиль для выделения кликом
                elem.className += " " + hoverClass;//назначаем класс для выделения строки по наведею мыши, т.к. курсор мыши в данный момент на строке, а выделение по клику уже снято
            }
            else //ряд не подсвечен
            {
                //если задана подсветка по наведению на строку, то убираем её
                if (hoverClass) elem.className = elem.className.replace(hoverClassReg, "");
                elem.className += " " + clickClass;//применяем класс подсветки по клику
                elem.setAttribute('clickedRow', true);//устанавливаем флаг того, что ряд кликнут и подсвечен
